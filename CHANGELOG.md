...
### Hinzugefügt
- **#69 Auto-Matching Engine (Kontoauszug ↔ Rechnung):** Neue Modul-Level-Funktion
  `_score_kontoauszug_gegen_rechnung(kb, rechnung)` berechnet Weighted Fuzzy Score (0–100):
  Betrag-Match 40 Punkte (±1 % Toleranz), Datum-Match 30 Punkte (±7 Tage),
  Text-Match 30 Punkte (Rechnungsnummer exakt/fuzzy via `difflib.SequenceMatcher`,
  Rechnungssteller-Wörter). `_auto_match_alle(conn, schwelle_auto, schwelle_vorschlag)`:
  verarbeitet alle unzugeordneten Ausgaben im Kontoauszug gegen alle offenen/teilbezahlten
  Rechnungen; Score ≥ 80 → automatische Buchung; Score 60–79 → Vorschlag für Review-Dialog;
  < 60 → bleibt offen. Keine externen Abhängigkeiten (difflib aus stdlib)

- **#70 MatchingReviewDialog:** Neuer Dialog zeigt Kontoauszugsbuchung links und
  Rechnungs-Kandidat rechts; visueller Score-Balken (grün/gelb/orange); bei mehreren
  Kandidaten Radio-Buttons zur Auswahl; Buttons: Bestätigen (Zahlung buchen + Rechnung
  verknüpfen), Ablehnen (Log-Eintrag abgelehnt), Überspringen, Fertig; Zusammenfassung
  am Ende (bestätigt / abgelehnt / übersprungen)

- **#71 kontoauszug_match_log DB-Tabelle:** GoB-konformer Audit Trail (§ 239 HGB):
  Jede automatische und manuell bestätigte Zuordnung wird mit kontoauszug_id, rechnung_id,
  zahlung_id, score, methode, erstellt_am, bestaetigt_am, abgelehnt gespeichert;
  neue Hilfsfunktion `_auto_log_match()` schreibt jeden Match-Vorgang

- **#72 Dashboard KPI: Auto-Match-Rate + Rechnungen:** Neue KPI-Zeile im Dashboard:
  „🔗 Auto-Matches" (Anzahl automatisch gebuchter Matches), „🧾 Rechnungen offen",
  „⏳ Teilbezahlt" — direkt sichtbar auf der Übersichtsseite

- **#74 Matching-Schwellwert in Einstellungen:** Neuer Tab „🔗 Matching" in EinstellungenPage
  mit konfigurierbaren Schwellwerten (matching_schwelle_auto, matching_schwelle_vorschlag);
  Matching-Log-Viewer zeigt kontoauszug_match_log als Tabelle (letzte 500 Einträge);
  Schwellwerte werden von `_auto_matching_starten()` aus Config gelesen

- **Button „🔗 Auto-Matching"** in KontoauszugPage-Kopfzeile: startet `_auto_match_alle()`,
  zeigt Fortschritt, öffnet MatchingReviewDialog für Vorschläge, zeigt Ergebnis-Zusammenfassung
...
### Behoben
- **Bug #A `zugferd_format` beim UPDATE verloren:** `_edit_rechnung` UPDATE-SQL um
  `zugferd_format` ergänzt; `RechnungDialog` speichert das erkannte Format in
  `_zugferd_format_val` und gibt es in `_on_save()` im Result-Dict zurück
- **Bug #B ZahlungDialog Rechnung-Dropdown:** Beim Bearbeiten einer Zahlung die einer
  bereits bezahlten/stornierten Rechnung zugeordnet ist, wird die Rechnung nun
  nachgeladen und korrekt im Dropdown angezeigt

### Hinzugefügt
- **#67 Auto-Status Rechnung (GoB):** Neue Hilfsfunktion `_auto_update_rechnung_status
  (conn, rechnung_id)` berechnet nach jeder Zahlungsänderung den korrekten Status der
  Rechnung (Offen → Teilbezahlt → Bezahlt) basierend auf Soll/Haben-Differenz;
  wird aufgerufen in `_new_zahlung`, `_edit_buchung` (auch für alte Rechnungs-ID)
  und `BuchungZuordnenDialog._toggle()`; überschreibt "Storniert" nie automatisch

- **#68 KI-OCR Fallback für Rechnungs-Import:** `_ki_ocr_rechnung_static(pdf_pfad)`
  sendet PDF an Anthropic claude-haiku-4-5-20251001 mit PDFs-Beta wenn weder
  ZUGFeRD noch xRechnung erkannt wurde; extrahiert alle Rechnungsfelder als JSON;
  Format wird als "PDF-KI-OCR" markiert; nur aktiv wenn Anthropic API-Key konfiguriert
